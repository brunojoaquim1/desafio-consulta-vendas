...
public interface SaleRepository extends JpaRepository<Sale, Long> {
    ...
    @Query(nativeQuery = true ,
           value = "SELECT TB_SELLER.NAME AS sellerName , " +
    ...
    @Query(nativeQuery = true ,
            ...
            countQuery = "SELECT COUNT(*) " +
                         ...
                         "INNER JOIN TB_SELLER ON TB_SELLER.ID = TB_SALES.seller_id " +
                         "WHERE TB_SALES.DATE BETWEEN :minDate AND :maxDate " +
                         "AND (:sellerName IS NULL OR UPPER(TB_SELLER.NAME) LIKE CONCAT('%', UPPER(:sellerName), '%')) " )
    Page<SallesReportProjection> sallesReport (LocalDate minDate, LocalDate maxDate, String sellerName, Pageable pageable );
